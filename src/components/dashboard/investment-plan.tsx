    ...
    <div className="rounded-2xl ring-1 ring-border/20 overflow-hidden bg-card">
      ...
      >
        <div className="flex items-center gap-2.5">
          <div className={`w-8 h-8 rounded-lg ${accentColor} flex items-center justify-center text-white shadow-sm`}>
            {icon}
          </div>
    ...
    <div className="space-y-3">
      ...
      <Card className="rounded-2xl border-0 shadow-lg ring-1 ring-border/20 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-950/20 dark:to-yellow-950/20 px-4 py-3 flex items-center gap-2.5 border-b border-border/30">
          ...
          <div>
            <h3 className="text-sm font-bold">خطة الاستثمار — حتى نهاية 2026</h3>
            <p className="text-xs text-muted-foreground">Investment Strategy — ذهب عيار 21</p>
          </div>
        </div>
        <CardContent className="p-4 space-y-4">
          {/* Executive Summary */}
          <div className="rounded-xl bg-gradient-to-br from-amber-50/80 to-yellow-50/50 dark:from-amber-950/20 dark:to-yellow-950/10 p-3.5 ring-1 ring-amber-200/50 dark:ring-amber-800/30">
            <div className="flex items-start gap-2.5">
              <div className="w-7 h-7 rounded bg-amber-400/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                <BookOpen className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
              </div>
              <div>
                <h4 className="text-xs font-bold text-amber-800 dark:text-amber-300 mb-1">الخلاصة التنفيذية</h4>
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2.5">
              ...
              <h4 className="text-xs font-bold">استراتيجية الشراء التدريجي (DCA)</h4>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              {dcaPhases.map((phase, idx) => (
                <motion.div
                  ...
                  className={`rounded-xl p-3 ${phase.bgLight} ring-1 ${phase.borderColor}`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    <div className={`w-7 h-7 rounded-lg bg-gradient-to-r ${phase.color} flex items-center justify-center text-white shadow-sm`}>
                      {phase.icon}
                    </div>
                    <div>
                      <span className="text-[10px] font-medium text-muted-foreground">المرحلة {phase.phase}</span>
                      <p className="text-xs font-bold">{phase.title}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className={`inline-flex items-center justify-center w-10 h-6 rounded bg-gradient-to-r ${phase.color} text-white text-xs font-black shadow-sm`}>
                      {phase.allocation}%
                    </span>
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2.5">
              ...
              <h4 className="text-xs font-bold">مناطق الشراء والبيع</h4>
            </div>
            <div className="rounded-xl ring-1 ring-border/20 overflow-x-auto">
              <table className="w-full text-xs min-w-[400px]">
                <thead>
                  <tr className="bg-muted/40">
                    <th className="py-2 px-3 text-right font-bold text-muted-foreground">المستوى (ج.م/جرام)</th>
                    <th className="py-2 px-3 text-right font-bold text-muted-foreground">الإجراء</th>
                    <th className="py-2 px-3 text-right font-bold text-muted-foreground">نسبة الميزانية</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      ...
                      >
                        <td className="py-2 px-3 font-mono tabular-nums">
                          ...
                          {isActive && (
                            <Badge variant="outline" className="mr-1.5 border-emerald-500 text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 text-[10px] px-1 py-0 rounded">
                              نشط
                            </Badge>
                          )}
                        </td>
                        <td className="py-2 px-3">
                          <span className={`${config.badgeBg} text-white text-[10px] font-bold px-2 py-0.5 rounded-md shadow-sm inline-flex items-center gap-1`}>
                            {config.icon}
                            {plan.action}
                          </span>
                        </td>
      ...
      >
        <div className="mt-3 space-y-1.5 max-h-64 overflow-y-auto custom-scrollbar">
          {priceTimeline.map((item, idx) => {
            ...
            return (
              <div key={idx} className={`flex items-center gap-2.5 py-1.5 px-2.5 rounded-lg ${
                ...
                <div className={`w-6 h-6 rounded flex items-center justify-center flex-shrink-0 ${
                  ...
                  {isSurge ? <TrendingUp className="w-3 h-3" /> : isDown ? <TrendingDown className="w-3 h-3" /> : <Minus className="w-3 h-3" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold">{item.period}</span>
                    <span className="text-xs font-mono font-bold tabular-nums">{item.price.toLocaleString()} ج.م</span>
                  </div>
                  {item.change && (
      ...
      >
        <div className="mt-3 space-y-2">
          <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-emerald-50/50 dark:bg-emerald-950/10">
            ...
            <div>
              <p className="text-xs font-bold">شراء البنك المركزي مستمر</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">التوترات الجيوسياسية وبنوك مركزية تشتري الذهب كأصل آمن — الدعم الأساسي مستمر</p>
            </div>
          </div>
          <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-green-50/50 dark:bg-green-950/10">
            <div className="w-6 h-6 rounded bg-green-100 dark:bg-green-900/30 flex items-center justify-center flex-shrink-0">
              <DollarSign className="w-3 h-3 text-green-600 dark:text-green-400" />
            </div>
            <div>
              <p className="text-xs font-bold">سعر الدولار أهم عامل محلي</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">كل حركة في الدولار تترجم فوراً إلى حركة مماثلة في سعر الذهب بالجنيه — نسبة ارتباط 80-90%</p>
            </div>
          </div>
          <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-teal-50/50 dark:bg-teal-950/10">
            <div className="w-6 h-6 rounded bg-teal-100 dark:bg-teal-900/30 flex items-center justify-center flex-shrink-0">
              <TrendingDown className="w-3 h-3 text-teal-600 dark:text-teal-400" />
            </div>
            <div>
              <p className="text-xs font-bold">خفض الفائدة الأمريكية</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">توقعات خفض الفائدة إلى 3.6% بنهاية 2026 — انخفاض الفائدة يعزز الذهب كمخزن للقيمة</p>
            </div>
          </div>
          <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-amber-50/50 dark:bg-amber-950/10">
            ...
            <div>
              <p className="text-xs font-bold">خفض فائدة البنك المركزي المصري</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">خفض الفائدة المركزية في فبراير 2026 إلى 19% — يدعم توجيه الأموال نحو أصول ذات عوائد أعلى</p>
            </div>
          </div>
        </div>
      </SectionToggle>
      ...
      >
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {/* Strengths */}
          <div className="rounded-xl bg-emerald-50/50 dark:bg-emerald-950/10 p-3 ring-1 ring-emerald-200/50 dark:ring-emerald-800/30">
            <h5 className="text-xs font-bold text-emerald-700 dark:text-emerald-400 mb-2 flex items-center gap-1.5">
          ...
          {/* Risks */}
          <div className="rounded-xl bg-red-50/50 dark:bg-red-950/10 p-3 ring-1 ring-red-200/50 dark:ring-red-800/30">
            <h5 className="text-xs font-bold text-red-700 dark:text-red-400 mb-2 flex items-center gap-1.5">
            ...
            <ul className="space-y-1.5">
              <li className="flex items-start gap-1.5 text-[11px] text-muted-foreground">
                ...
                <span>التصحيح قد يتعمق — عيار 12 قد ينخفض لأقل من 5,500 ج.م</span>
              </li>
              <li className="flex items-start gap-1.5 text-[11px] text-muted-foreground">
                <span className="w-1.5 h-1.5 rounded-full bg-red-500 mt-1 flex-shrink-0" />
                <span>استمرار التصحيح قد يكسر الذهب — ضغط على السعر المحلي</span>
              </li>
              <li className="flex items-start gap-1.5 text-[11px] text-muted-foreground">
                <span className="w-1.5 h-1.5 rounded-full bg-red-500 mt-1 flex-shrink-0" />
      ...
      >
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
          {goldRules.map((rule, idx) => (
            <div key={idx} className="flex items-start gap-2 p-2 rounded-lg bg-muted/20 hover:bg-muted/40 transition-colors">
              <div className="w-6 h-6 rounded bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center flex-shrink-0 text-amber-600 dark:text-amber-400">
                {rule.icon}
              </div>
              <div>
                <p className="text-xs font-bold">{rule.title}</p>
                <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">{rule.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          ...
          <div className="rounded-xl bg-red-50/50 dark:bg-red-950/10 p-3 ring-1 ring-red-200/50 dark:ring-red-800/30">
            ...
            <ul className="space-y-1.5">
              ...
              <li className="flex items-start gap-1.5 text-[11px] text-muted-foreground">
                <TrendingDown className="w-3 h-3 text-red-500 mt-0.5 flex-shrink-0" />
                <span>من يتوقع عافية قوية للجنيه المصري</span>
              </li>
              <li className="flex items-start gap-1.5 text-[11px] text-muted-foreground">
                <TrendingDown className="w-3 h-3 text-red-500 mt-0.5 flex-shrink-0" />
      ...
      <motion.div
        ...
        className="rounded-2xl bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-950/20 dark:to-green-950/20 p-3.5 ring-2 ring-emerald-300/50 dark:ring-emerald-700/30 shadow-lg"
      >
        <div className="flex items-start gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-r from-emerald-500 to-green-600 flex items-center justify-center shadow-sm flex-shrink-0">
            <Zap className="w-4 h-4 text-white" />
          </div>
          <div>
            <h4 className="text-xs font-bold text-emerald-800 dark:text-emerald-300 mb-1">نصائح فورية للتنفيذ</h4>
            <div className="space-y-1">
              <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                <ArrowRight className="w-2.5 h-2.5 text-emerald-500 flex-shrink-0" />
  ...
  if (!effectiveSignal) {
    return (
      <Card className="rounded-2xl border-0 shadow-lg ring-1 ring-border/20 overflow-hidden h-full">
        ...
        <div className="bg-gradient-to-r from-muted/50 to-muted/30 px-4 py-3 flex items-center gap-2.5 border-b border-border/30">
